...
const UnitPrice = ({name, value, handleUnitPrice, handleUnitName , isOpenEdit, isNew, onClick})=>{
    ...
    <tr>
      {
        isNew ? 
        <td>
          <input className='form-input' onChange={handleUnitName} name={name} placeholder='Tipo de unidad'/>
        </td> 
        : 
        <td>
            <p>{name}</p>
...
const ProfileNewProduct = (props)=>{
  ...
  const handleChange = (e) =>{
    setInfoProduct({
      ...infoProduct,[e.target.name]: e.target.value,
    })
  }
  ...
  const handleUnitPrice = (e) =>{
    ...
    setInfoProduct({
      ...infoProduct,
      prices: infoProduct.prices.map(item => item.name !== e.target.name ? item : {...item,value:e.target.value} )
    })
  }
  ...
  const insertNewPrice = () =>{
    setInfoProduct({
      ...infoProduct,
      prices: [...infoProduct.prices,{name:String(infoProduct.prices.length),value:0, isNew:true, isOpenEdit:false}]
    })
  }
  ...
  const handleUnitName = (e)=> {
    setInfoProduct({
      ...infoProduct,
      prices: infoProduct.prices.map((item,index) => item.name !== e.target.name ? item : {...item,name:e.target.value})
    })
  }
  ...
  const openEdit = (e)=>{
    ...
    setInfoProduct({
      ...infoProduct,
      prices: infoProduct.prices.map((item,index) => item.name !== e.target.name ? item : {...item, isOpenEdit:true})
    })
  }
          ...
          <div className="newProduct__info">
            ...
            <div className="l-systemSubGroup">
              ...
              <div className="form-group">
                ...
                <input className="form-input" name='name' type="text" placeholder="Nombre del producto" onChange={handleChange} />
              </div>

              <div className="form-group">
                <label htmlFor="">Descripcion</label>
                <textarea className="form-textArea" cols="10" rows="10" name='description' type="text" placeholder="Descripcion del producto" onChange={handleChange} />
              </div>

              <div className="form-group">
                <label htmlFor="">Disponibilidad</label>
                <select className='form-listBox' name='avaliable' onChange={handleChange}>
            ...
            <div className="l-systemSubGroup">
              ...
              <div className="form-group newProduct__unitPrices">
                  ...
                  <table>
                      ...
                      <colgroup>
                        <col className='unitsName' />
                        <col className='unitsPrice'/>
                        <col className='test'/>
                      </colgroup>
                    ...
                    <tbody>
                      ...
                      {
                        infoProduct.prices.map((item,index)=><UnitPrice key={index} {...item} handleUnitPrice={handleUnitPrice} handleUnitName={handleUnitName} onClick={openEdit}/>)
                      }

                      <tr>
                        <td className='newProduct__addUnitPrice' colSpan='3'>
                        <div className="button button--icon" onClick={insertNewPrice}>
                          <svg className='button__icon' width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect y="8.65381" width="2.88462" height="15" rx="1.44231" transform="rotate(-90 0 8.65381)" fill="#2EC4B6"/>
                            <rect x="6.34607" width="2.88462" height="15" rx="1.44231" fill="#2EC4B6"/>
                          </svg>
